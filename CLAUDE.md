...
- `src/core/embedding.ts` — OpenAI text-embedding-3-large, batch, retry, backoff. v0.27.0: SDK constructor now passes `apiKey: getSecret('OPENAI_API_KEY')` instead of relying on the SDK's env-var auto-pickup, so users with a configured `secrets.openai_api_key_command` (Keychain/pass/op) keep the key out of `process.env`.
- `src/core/secrets.ts` (v0.27.0) — module-private secret resolver. `getSecret('OPENAI_API_KEY' | 'ANTHROPIC_API_KEY' | 'GROQ_API_KEY')` resolves to a configured `~/.gbrain/config.json` `secrets.<name>_command` (run via `execSync` with 10s timeout), falling back to `process.env[name]`. Resolved values cached in a process-lifetime `Map`; never written to `process.env` so they don't inherit into subprocesses. A configured-but-broken command throws loudly with stderr — no silent fallback to env (would mask Keychain ACL bugs). `clearSecretsCache()` is test-only; `resolveAllConfiguredSecrets()` powers the `gbrain doctor` `secret_resolution` check (reports per-key success/failure without exposing values).
